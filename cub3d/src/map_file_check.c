...
/*   Updated: 2024/04/03 12:07:58 by akurmyza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool map_file_check(t_game *game, int argc, char **argv)
{
	...
	if (argc != 2)
		ft_error_exit_game(game, "Missing map file: <map_name>.cub");
	...
	{
		free(temp_file);
		ft_error_exit_game(game, "Expected file: <map_name>.cub");
	}
	else if (is_substring(".cub", temp_file, length_temp_file - 4, 4) == FALSE)
	{
		free(temp_file);
		ft_error_exit_game(game, "Invalid  file extension: .cub expected");
	}
...
t_bool is_valid_file(t_game *game, char *temp_file)
{
	...
	if (bytes_read == -1)
	{
		free(temp_file);
		close(fd);
		ft_error_exit_game(game, "Error reading file");
	}
	if (bytes_read == 0)
	{
		free(temp_file);
		close(fd);
		ft_error_exit_game(game, "Error: File is empty");
	}
	close(fd);
	return (TRUE);
}
